...
function build() {
  ...
  TOK_TERMS.forEach((term) => {
    reflections[term] = [];

    for (let i = 0; i < depth; i++) {
      reflections[term].push(generateReflection(term, i));
    }
  });
...
function generateReflection(term, stage) {
  const tones = [
...
function download() {
  ...
  const name = `${data.studentProfile.firstName || "student"}_${data.spiral.reflectionDepth}term.json`;
